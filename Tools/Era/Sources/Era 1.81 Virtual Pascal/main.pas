...
PROCEDURE CallProc(Addr: INTEGER; Conversion: INTEGER; PParams: POINTER; NumParams: INTEGER); ASSEMBLER; {$FRAME+}
CONST
...
ASM
  MOV IsFloatRes, 0
  CMP Conversion, CONVENTION_FLOAT
  JB @@IntConvention
@@FloatConvetion:
  MOV IsFloatRes, 1
  SUB Conversion, CONVENTION_FLOAT
@@IntConvention:
...
BEGIN
  ...
  WHILE CmdStr^ <> ';' DO BEGIN
    ...
    CASE Cmd OF 
      ...
      END; // .switch Q
      'E': BEGIN
        // PARAMS: Addr: POINTER; Conversion: INTEGER; Params: ANY...
        IF
          (NumParams < 0) OR
      ...
      'X': BEGIN
        ...
        FOR i:=0 TO NumParams - 1 DO BEGIN
          ...
          END // .if
          ELSE BEGIN
            General.EventParams[i]:=Params[i].Value;
          END; // .else
        END; // .for
      END; // .switch X
    ELSE
